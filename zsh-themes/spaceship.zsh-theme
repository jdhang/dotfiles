...
# N
SPACESHIP_N_SHOW="${SPACESHIP_N_SHOW:-true}"
SPACESHIP_N_SYMBOL="${SPACESHIP_N_SYMBOL:-⬢}"

# RUBY
SPACESHIP_RUBY_SHOW="${SPACESHIP_RUBY_SHOW:-false}"
...
# Show current version of node, exception system.
spaceship_nvm_status() {
  [[ $SPACESHIP_N_SHOW == false ]] && return

  local n_status=$(node -v 2>/dev/null)
  # [[ "${n_status}" == "system" ]] && return
  n_status=${n_status}

  echo -n " %Bvia%b "
  echo -n "%{$fg_bold[green]%}"
  echo -n "${SPACESHIP_N_SYMBOL} ${n_status}"
  echo -n "%{$reset_color%}"
}
